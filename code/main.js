...
client.once("ready", () => {
    console.log(
        functions.get_formatted_date() +
            'BOT "Translation please" is now ready!\n'
    );
...
client.on("message", async (message) => {
    ...
    fs.mkdirSync([".", "debug", message.guild.name].join("/"), {
        recursive: true,
    });
    fs.appendFile(
    ...
    if (fs.existsSync(`./temp/${message.author.id}.txt`)) {
        ...
        for (let dictionary_name of dictionaries) {
            ...
            if (user_stats[dictionary_name] == undefined)
                user_stats[dictionary_name] = { right: 0, wrong: 0 };
        }

        let file_contents = fs
            .readFileSync(`./temp/${message.author.id}.txt`, "utf8")
            .split("\r\n");
        ...
        if (isValid) {
            message.reply(
                `Congratulations! You found the right word, \`${valid_words}\` !`
            );
